  ...
  <div class="container">
    ...
    <div v-if="passing">
      <div v-if="passing.length > 0">
        ...
        <button class="btn btn-success mt-2 mr-2" @click="exportJson">
          Экспорт в .json
        </button>
        <download-excel
          :data="json_data"
      ...
      <b-alert
        ...
        >Результаты появятся после того, как хотя бы 1 пользователь пройдет
        тест</b-alert
      >
    </div>
  </div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch(actionTypes.getPassingTest).then(() => {
      let temp = [];
